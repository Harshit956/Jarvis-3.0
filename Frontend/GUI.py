...
def SetMichrophoneStatus(Command):
    with open(rf"{TempDirPath}\Mic.data", "w", encoding="utf-8") as file:
        file.write(Command)

def GetMichrophoneStatus():
...
def MicButtonInitialed():
    SetMichrophoneStatus("False")

def MicButtonClosed():
    SetMichrophoneStatus("True")
...
class ChatSection(QWidget):
    ...
    def laod_icon(self, path, width=60, height=60):
        pixmap = QPixmap(path)
    ...
    def toggle_icon(self, event=None):
        ...
        if self.toggled:
            self.laod_icon(GraphicsDirectoryPath("voice.png"), 60, 60)
            MicButtonInitialed()
