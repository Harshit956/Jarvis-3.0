...
async def TextToAudioFile(text) -> None:
    ...
    if os.path.exists(file_path): # Check if the file already exists.
        os.remove(file_path) # If it exists, remove it to avoid overwrite errors.
...
def TTS(Text, func=lambda r=None: True):
    while True:
        try:
            # Convert text to an audio file aynchronously.
            ...
            try:
                # Call the provided function with Fasle to signal the end of TTS.
                func(False)
                pygame.mixer.music.stop() # Stop the audio playback.
                pygame.mixer.quit() # Quit the pygame mixer.

            except Exception as e: # Handle any exceptions during clean-up.
                print(f"Error in finally block: {e}")

# Fucntion to manage text-to-speech with additional responses for long text.
