...
# Initilize the Groq client using the API key.
...
# Attempt to laod the chat log from a JSON file.
try:
...
# Function to modify the chatbot's response for better formattig.
...
def ChatBot(Query):
    """ This function sends the user's query to the chatbot and returns the AI's resposne. """
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            stream=True, # Enable streaming resposne.
            stop=None # Allow model to determine when to stop.
        )

        # Initilize an empty string to store the AI's response.
        Answer = ""
        ...
        with open(r"Data\ChatLog.json", "w") as f:
            dump([], f, indent=4)
        return ChatBot(Query) # Retry the query after resetting the chatlog.
