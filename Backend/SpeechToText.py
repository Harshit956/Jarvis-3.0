...
# Function to modify a query to ensure proper puntuation and functioning.
def QueryModifier(Query):
    new_query = Query.lower().strip()
    query_words = new_query.split()
    question_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whom", "can you", "what's", "where's", "how's"]

    # Check if the query is a question and add a question mask if necessary.
    if any(word + " " in new_query for word in question_words):
        ...
        # Add a period is the query is not a question.
        if query_words[-1][-1] in [".", "?", "!"]:
            new_query = new_query[:-1] + "."
        else:
            new_query += "."
...
def SpeechRecognition():
    ...
    # Start Speech Recognition using the start button.
    ...
    while True:
        try:
            ...
            if Text:
                # Stop the recogition by using the stop button.
                ...
                if InputLanguage.lower() == "en" or "en" in InputLanguage.lower():
                    ...
                    # If the input lnaguage is not english, translate it and return text.
